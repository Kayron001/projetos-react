        ...
        return (
            <Paper className={classes.paper} raised elevation={6}>
                <Typography variant='h6' align='center'>
                    Faça o login para poder criar suas memórias
                </Typography>
            </Paper>
        )
    }
    ...
    return (
        <Paper className={classes.paper} raised elevation={6}>
            <form autoComplete='off' noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
                <Typography variant='h6'>{currentId ? 'Editando' : 'Criando'} uma Memória</Typography>
                ...
                <TextField name='message' variant='outlined' label='Mensagem' fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
                <TextField name='tags' variant='outlined' label='Hashtags (Separadas por vírgula)' fullWidth value={postData.tags} onChange={(e) => setPostData({ ...postData, tags: e.target.value.split(',') })} />
