...
export const updateVideo = async (req, res, next) => {
    try {
        ...
        if (req.user.id === video.userId) {
            ...
            next(createError(403, 'Você pode atualizar apenas seus vídeos'))
        }
    } catch (error) {
        next(error)
    }
...
export const deleteVideo = async (req, res, next) => {
    try {
        ...
        if (req.user.id === video.userId) {
            ...
            res.status(200).json("O vídeo foi deletado com sucesso!")
        } else {
            next(createError(403, 'Você pode deletar apenas seus vídeos'))
        }
    } catch (error) {
        next(error)
    }
...
export const updateViews = async (req, res, next) => {
    try {
        ...
        res.status(200).json('Visualizações aumentadas')
    } catch (error) {
        next(error)
    }
}
